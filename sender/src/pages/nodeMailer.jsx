...
const NodeMailer = () => {
  ...
  const [activeFirstNameAccordion, setActiveFirstNameAccordion] =
    useState(null);

  const [activeLastNameAccordion, setActiveLastNameAccordion] = useState(null);
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        // Skip the header line by starting from index 1
        ...
        if (validateEmails(emailArray)) {
          ...
          setLastNames(lastNameArray);
          setSelectedFile(file);
          setSelectedFirstName(file);
          setSelectedLastName(file);

          setEmailCount(emailArray.length);
          setFirstNameCount(firstNameArray.length);
          setLastNameCount(lastNameArray.length);
        } else {
  ...
  const validateEmails = (emails) => {
    ...
    emails.forEach((email, index) => {
      if (!emailRegex.test(email)) {
        console.error(`Invalid email found at index ${index}: ${email}`);
        allValid = false;
      }
    });
  ...
  const toggleFirstNameAccordion = (index) => {
    setActiveFirstNameAccordion(
      activeFirstNameAccordion === index ? null : index
    );
  };

  const toggleLastNameAccordion = (index) => {
    setActiveLastNameAccordion(
      activeLastNameAccordion === index ? null : index
    );
  };
  ...
  const handleAttachmentUpload = async (event) => {
    ...
    reader.onload = (e) => {
      ...
      setAttachmentName((prevAttachmentName) => [
        ...prevAttachmentName,
        file.name,
      ]);
      ...
      localStorage.setItem(
        "attachmentName",
        JSON.stringify([...attachmentName, file.name])
      );
      localStorage.setItem(
        "attachmentData",
        JSON.stringify([...attachmentData, fileData])
      );
  ...
  const handleDeleteAttachment = (index) => {
    const updatedAttachmentName = [...attachmentName];
    updatedAttachmentName.splice(index, 1);

    const updatedAttachmentData = [...attachmentData];
    updatedAttachmentData.splice(index, 1);
    ...
    localStorage.setItem(
      "attachmentName",
      JSON.stringify(updatedAttachmentName)
    );
    localStorage.setItem(
      "attachmentData",
      JSON.stringify(updatedAttachmentData)
    );
  };
  //////////
  ...
  const sendEmail = async (e) => {
    ...
    try {
      ...
      // Section 1-a: First Names
      setFirstNames([]);
      setSelectedFirstName(null);
      setFirstNameCount(0);

      // Section 1-b: Last Names
      setLastNames([]);
                  ...
                  <div className="relative p-6 mt-4 bg-white border-t-2 border-blue-700 rounded-lg shadow sm:p-5">
                    <div className="p-6 bg-white border border-gray-300 shadow-lg rounded-xl">
                      ...
                      <p className="mb-2 text-sm">
                        ...
                        add each email in your lead to a letter.
                      </p>
                      <p className="mb-2 text-sm">
                        - Add this: "{firstName}" to your letter if you want to
                        add first names to each letter.
                      </p>
                      <p className="mb-2 text-sm">
                        - Add this: "{lastName}" to your letter if you want to
                        add last names to each letter.
                      </p>
                      <p className="mt-6 text-sm italic">
                        Make sure to add them without the quotes wherever you
                        want them to be displayed in your letter. So, don't add{" "}
                        <strong>"{userEmail}"</strong>, instead add{" "}
                    ...
                    <form className="mt-8">
                      ...
                      <div className="grid gap-4 pt-8 mb-4 sm:grid-cols-2">
                        <div className="mt-0">
                          ...
                          <input
                            ...
                            type="file"
                            accept=".txt, .csv"
                            //onChange={handleFirstNameUpload}
                          />
                          */}
                          </>
                          ...
                          <div id="accordion-flush">
                            ...
                            <div>
                              <button
                                type="button"
                                onClick={() => toggleFirstNameAccordion(1)}
                                className={`flex items-center justify-between w-full py-2 mt-1 font-medium rtl:text-right border-b text-gray-900 border-gray-700 gap-3 ${
                                  activeFirstNameAccordion === 1
                                    ? "bg-transparent"
                                    : ""
                                }`}
                              >
                                ...
                                >
                                  [{firstNameCount.toLocaleString()} first names
                                  uploaded]
                                </p>
                                <svg
                                  className={`w-3 h-3 rotate-180 shrink-0 ${
                        ...
                        */}
                          ...
                          <div id="accordion-flush">
                            ...
                            <div>
                              <button
                                type="button"
                                onClick={() => toggleLastNameAccordion(1)}
                                className={`flex items-center justify-between w-full py-2 mt-1 font-medium rtl:text-right border-b text-gray-900 border-gray-700 gap-3 ${
                                  activeLastNameAccordion === 1
                                    ? "bg-transparent"
                                    : ""
                                }`}
                              >
                                ...
                                >
                                  [{lastNameCount.toLocaleString()} last names
                                  uploaded]
                                </p>
                                <svg
                                  className={`w-3 h-3 rotate-180 shrink-0 ${
                        ...
                        <div className="grid gap-4 pt-8 mb-4 sm:grid-cols-2">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5"
                              placeholder="Your Complaint to Chase Bank - - Our Ref: CMOS-03294239"
                            />
                          </div>
                        </div>

                        <p className="pt-8 text-sm text-gray-600">
                          Add any attachment you would like to use together with
                          your letter
                        </p>
                      ...
                      <button
                        type="button"
                        onClick={sendEmail}
                        className="inline-flex w-full justify-center mt-10 items-center px-5 py-2.5 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
